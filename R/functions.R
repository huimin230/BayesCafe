...
##
## Project      : BayesCafe
## Research goal: Spatial clustering with feature selection
## Modified     : 2023-02-22
##
################################################################################
...
switch_label <- function(u_store, z_store, gamma_store, K){
  ...
  if (is.na(which(colSums(gamma_store) == iter)[1])) {
    index <- which(colSums(gamma_store[(iter/2+1):iter, ]) == iter/2*0.5)[1]
  }

  for (i in 1:iter){
    z_temp <- z_store[i,] + 1
    u_temp <- matrix(u_store[, ,i], ncol = dim(u_store)[2])
...
plot.cluster <- function(data, x, y, size = 4, cluster, label = "Cluster", colors = NULL, title = NULL, ncol = 1){
...
plot.pve <- function(data, x, y, group=NULL, caption=NULL) {
  if (length(unique(data$group)) == 4){
    colors = c("green", "#00AFBB", "#E7B800", "#FC4E07")
  }else{
  ...
  p <- ggplot(data, aes(x={{x}}, y={{y}}, group={{group}}, color={{group}})) + 
    ...
    labs(caption = caption, color=NULL) +
    scale_color_manual(values=colors) +
    theme(plot.caption = element_text(hjust=0.5)) +
    theme(legend.position = c(0.003, 0.996),
...
plot.density <- function(data, x, group=NULL, caption=NULL) {
  p <- ggplot(data, aes(x={{x}}, group={{group}}, color={{group}})) + 
    geom_density() +
    ylab("Density") +
    theme(legend.key=element_rect(fill="white")) +
    theme_minimal() +
...
plot.trace <- function(data, x, y, group=NULL, ylab, caption=NULL) {
  p <- ggplot(data, aes(x={{x}}, y={{y}}, group={{group}}, color={{group}})) + 
    geom_line() +
    ylab(ylab) +
    theme(legend.key=element_rect(fill="white")) +
...
plot.ppi <- function(data_type, data, x, y, z=NULL, caption=NULL) {
  if (data_type == "real") {
...
# plot.ppi <- function(data_type, data, z=NULL, caption=NULL) {
#   data <- data[order(data$PPI, decreasing = TRUE), ]
#   if (data_type == "real") {
#     p <- ggplot(data, aes(x=1:nrow(data), y=PPI)) +
#       geom_point()
# 
#   } 
#   
#   else if (data_type == "simulated"){
#     p <- ggplot(data, aes(x=1:nrow(data), y={{z}})) +
#       geom_point(color="red") +
#       geom_segment(aes(x={{x}},xend={{x}},y=0, yend={{y}}))
#   }
# 
...
#     geom_hline(yintercept = 1-c, colour = "blue", linetype=2) +
#     xlab("Gene index") +
#     ylab("Posterior probabilities of inclusion") +
#     labs(caption = caption) +
#     theme(plot.caption = element_text(hjust=0.5))
