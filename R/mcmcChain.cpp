...
// [[Rcpp::export]]
Rcpp::List mcmc(arma::mat Y, IntegerVector z, IntegerVector gamma, NumericVector s, arma::mat P, bool zinb, int iter, int burn) {
  ...
  NumericVector mse_store(iter);   // to store mse between count and estimated count
  ...
  for (k = 0; k < K; k++){
    for (j = 0; j < p; j++){
      if (gamma(j) == 1) {
        arma::uvec indices = which(z, k);
        arma::vec column = Mu.col(j);
        arma::vec row = column(indices);
        if (mean(row)!=0){
          u(k, j) = mean(row);
  ...
  for (t = 0; t < iter; t++) {
    ...
    // Calculate MSE of count (Y) and estimated count
    double mse = 0;
    
    for (i = 0; i < n; i++) {
      for (j = 0; j < p; j++) {
        double size = phi(j);
        double prob = size/(size + u(z(i), j));
        if (H(i, j) == 1){
          Y_hat(i, j) = 0;
        }else {
          Y_hat(i, j) = rnbinom(1, size, prob)(0);
        }
        
        mse = mse + pow(Y(i,j ) - Y_hat(i, j), 2);
      }
    }
  ...
  return Rcpp::List::create(Rcpp::Named("Mu") = Mu, Rcpp::Named("mse_store") = mse_store, 
                            ...
                            Rcpp::Named("u_store") = u_store, Rcpp::Named("phi_store") = phi_store, 
